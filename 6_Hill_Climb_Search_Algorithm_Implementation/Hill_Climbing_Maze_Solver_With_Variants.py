...
class MazeSolver:
            ...
            for neighbor in neighbors:
                if neighbor not in visited:
                    ...
                    if dist < current_dist:
                        better_neighbors.append((neighbor, dist))

            if not better_neighbors:
                print("Stuck! No better neighbors")
                break

            selected = random.choice(better_neighbors)
            next_node = selected[0]
            next_dist = selected[1]

            path.append(next_node)
            visited.add(next_node)
            current = next_node

            print(f"Moving to {current}, Distance: {next_dist}")
    ...
    def solve(self):
        ...
        path3, steps3 = self.stochastic_hill_climbing()
        self.print_solution(path3, steps3, "Stochastic Hill Climbing")
