...
def heuristic_cost(graph, current, goal):
    return math.sqrt(
        (graph[goal].heuristic[0] - graph[current].heuristic[0])**2 +
        (graph[goal].heuristic[1] - graph[current].heuristic[1])**2
    )

def steepest_hill_climbing(initialState, goalState):
    graph = {
    ...
    while current != goalState:
        current_cost = heuristic_cost(graph, current, goalState)

        neighbors = []
        for child in graph[current].actions:
            child_name = child[0]
            cost = heuristic_cost(graph, child_name, goalState)
            neighbors.append((child_name, cost))
...
result = steepest_hill_climbing('A', 'Y')
print(result)
