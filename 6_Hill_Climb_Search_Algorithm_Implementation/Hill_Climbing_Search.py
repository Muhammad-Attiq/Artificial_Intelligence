...
def hillClimbing(initialState, goalState):
    graph = {
    ...
    parentCost =
    math.sqrt((graph[goalState].heuristic[0] -
    ...
    while parentNode != goalState:
        ...
        for child in graph[parentNode].actions:
            ...
            if child_name not in explored:
                childCost =
                math.sqrt((graph[goalState].heuristic[0]
                           - graph[child_name].heuristic[0])**2 +
(graph[goalState].heuristic[1] - graph[child_name].heuristic[1])**2)
